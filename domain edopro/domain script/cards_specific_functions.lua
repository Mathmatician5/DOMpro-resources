...
function Auxiliary.AddMaleficSummonProcedure(c,code,loc,excon)
	...
	e1:SetRange(LOCATION_HAND|LOCATION_DECK_MASTER_ZONE)
...
function Auxiliary.FossilLimit(e,se,sp,st)
	return not e:GetHandler():IsLocation(LOCATION_EXTRA|LOCATION_DECK_MASTER_ZONE) or se:GetHandler():IsCode(CARD_FOSSIL_FUSION)
end
...
function Infernoid.RegisterSummonProcedure(c,monstersToBanish)
	...
	e2:SetRange(monstersToBanish>1 and (LOCATION_HAND|LOCATION_GRAVE|LOCATION_DECK_MASTER_ZONE) or (LOCATION_HAND|LOCATION_DECK_MASTER_ZONE))
