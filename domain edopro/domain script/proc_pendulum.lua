...
function(c,reg,desc)
	...
	if reg==nil or reg then
		...
		if c:IsType(TYPE_FUSION|TYPE_SYNCHRO|TYPE_XYZ|TYPE_LINK) then
			...
			e2:SetRange(LOCATION_HAND|LOCATION_DECK_MASTER_ZONE)
		end
		c:RegisterEffect(e2)
	end
...
function Pendulum.Filter(c,e,tp,lscale,rscale,lvchk)
	...
	return (c:IsLocation(LOCATION_HAND) or (c:IsLocation(LOCATION_DECK_MASTER_ZONE) and (not c:IsType(TYPE_EXTRA)) and ((not c:IsType(TYPE_RITUAL)) or c:IsType(TYPE_PENDULUM))) or (c:IsLocation(LOCATION_EXTRA) and c:IsFaceup() and c:IsType(TYPE_PENDULUM)))
...
function Pendulum.Condition()
	return	function(e,c,inchain,re,rp)
				...
				local loc=0
				if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then loc=(loc|LOCATION_HAND|LOCATION_DECK_MASTER_ZONE) end
...
function Pendulum.Operation()
	return	function(e,tp,eg,ep,ev,re,r,rp,c,sg,inchain)
				...
				local loc=0
				if ft1>0 then loc=(loc|LOCATION_HAND|LOCATION_DECK_MASTER_ZONE) end
				...
				ft1=math.min(ft1,tg:FilterCount(Card.IsLocation,nil,LOCATION_HAND|LOCATION_DECK_MASTER_ZONE))
				ft2=math.min(ft2,tg:FilterCount(Card.IsLocation,nil,LOCATION_EXTRA))
				ft2=math.min(ft2,aux.CheckSummonGate(tp) or ft2)
				while true do
					local ct1=tg:FilterCount(Card.IsLocation,nil,LOCATION_HAND|LOCATION_DECK_MASTER_ZONE)
					...
					if sg:IsContains(tc) then
						sg:RemoveCard(tc)
						if tc:IsLocation(LOCATION_HAND|LOCATION_DECK_MASTER_ZONE) then
							ft1=ft1+1
						else
							ft2=ft2+1
						end
						...
						if c:IsHasEffect(511007000)~=nil or rpz:IsHasEffect(511007000)~=nil then
							if not Pendulum.Filter(tc,e,tp,lscale,rscale) then
								...
								local ct0,ct3,ct4=#pg,pg:FilterCount(Card.IsLocation,nil,LOCATION_HAND|LOCATION_DECK_MASTER_ZONE),pg:FilterCount(Card.IsLocation,nil,LOCATION_EXTRA)
								sg:Sub(pg)
								ft1=ft1+ct3
								ft2=ft2+ct4
								ft=ft+ct0
								...
								sg:Sub(pg)
								if #pg>0 then
									if pg:GetFirst():IsLocation(LOCATION_HAND|LOCATION_DECK_MASTER_ZONE) then
										ft1=ft1+1
									else
										ft2=ft2+1
									end
