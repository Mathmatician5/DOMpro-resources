...
function(c,_type,filter,lv,desc,extrafil,extraop,matfilter,stage2,location,forcedselection,customoperation,specificmatfilter,requirementfunc,sumpos,extratg)
	...
	if location and (location & (LOCATION_HAND|LOCATION_EXTRA)) then
		location=location|LOCATION_DECK_MASTER_ZONE
	end
...
local function GetDefaultSummonFromLocation()
	return Duel.IsDuelType(DUEL_EXTRA_DECK_RITUAL) and (LOCATION_EXTRA|LOCATION_DECK_MASTER_ZONE) or (LOCATION_HAND|LOCATION_DECK_MASTER_ZONE)
end
