...
function Synchro.AddProcedure(c,...)
	...
	e1:SetRange(LOCATION_EXTRA|LOCATION_DECK_MASTER_ZONE)
...
function Synchro.CheckP43(tsg,ntsg,sg,lv,sc,tp)
	...
	and ((sc:IsLocation(LOCATION_EXTRA|LOCATION_DECK_MASTER_ZONE) and Duel.GetLocationCountFromEx(tp,tp,sg,sc)>0)
		or (not sc:IsLocation(LOCATION_EXTRA|LOCATION_DECK_MASTER_ZONE) and Duel.GetMZoneCount(tp,sg,tp)>0))
end
...
function Synchro.MajesticCheck2(sg,card1,card2,card3,lv,sc,tp,f1,cbt1,f2,cbt2,f3,cbt3,...)
	...
	if sc:IsLocation(LOCATION_EXTRA|LOCATION_DECK_MASTER_ZONE) then
